...
public func imCallTitle(_ state: IMCallViewState, resolver: IMProfileResolving? = nil) -> String {
    if state.isMeeting { return state.roomID.isEmpty ? imT("call.meeting") : imT("call.meetingRoom", ["room": state.roomID]) }
    if state.isGroup { return imT("call.group", ["n": state.participants.count + 1]) }
    return state.peerUID.isEmpty ? imT("call.default") : imResolvedName(resolver, uid: state.peerUID, fallback: state.peerUID)
}
...
public func imCallStatusLine(_ state: IMCallViewState, now: TimeInterval = Date().timeIntervalSince1970) -> String {
    ...
    switch state.phase {
    case .incoming:   return imT(state.mediaType == "video" ? "incoming.video" : "incoming.audio")
    case .outgoing:   return imT("call.status.calling")
    case .connecting: return imT(state.isMeeting ? "call.status.enteringMeeting" : "call.status.connecting")
    case .ended:
        // 时长用服务端给的那个（不变量 I8）。现算的话，没接通的通话 beganAt 是 0，
        // 算出来是一九七〇年到现在的秒数。
        return state.isMeeting ? imT("end.meetingLeft")
