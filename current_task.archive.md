...
## 2026-09-15（第三轮）：四端 API 命名对齐（已提交 `c74d1be`）
...
0. **公网发布相关（本轮遗留）**：
   - ~~tag 后改 exactVersion~~：2026-09-17 tag `1.0.0` 已推，`IMRTCDemo.xcodeproj` 已改 `exactVersion 1.0.0`，`xcodebuild -project` 解析到 `1.0.0`（= tag 所指提交）并在模拟器上编过。
   ...
   - 用户 09-17 已在 Xcode 里打开过 workspace 并装真机，但 workspace 的 `xcshareddata/swiftpm/Package.resolved` 仍没生成——可能 Xcode 把本地覆盖场景的解析结果只写在 DerivedData，不必再追。
...
## 2026-09-19（快照整理时移出活快照）：09-18～09-17 的「当前焦点」与旧「下一步」
